...
def render_scene(samples_per_pixel):
    # Set Scene
    ...
    # Cornell Box Scene
    # Back Wall
    Sc.add(Plane(material=white_diffuse, center=vec3(0, 0, 10), width=120, height=120, u_axis=vec3(5, 0, 0),
                 v_axis=vec3(0, 5, 0)))

    # Floor
    Sc.add(Plane(material=white_diffuse, center=vec3(0, -10, 0), width=120, height=120, u_axis=vec3(5, 0, 0),
                 v_axis=vec3(0, 0, 5)))
    ...
    # Left Wall
    Sc.add(Plane(material=red_diffuse, center=vec3(-10, 0, 0), width=120, height=120, u_axis=vec3(0, 5, 0),
                 v_axis=vec3(0, 0, 5)))

    # Right Wall
    Sc.add(Plane(material=mirror, center=vec3(10, 0, 0), width=120, height=120, u_axis=vec3(0, 5, 0),
                 v_axis=vec3(0, 0, 5)))
    ...
    img = Sc.render(samples_per_pixel=samples_per_pixel)
    end_time = time.time()

    img.save(f"EXAMPLE_{samples_per_pixel}.png")
    print(f"Rendering with {samples_per_pixel} samples_per_pixel took {end_time - start_time:.2f} seconds")
    return img

if __name__ == "__main__":
    # List of samples_per_pixel to render in parallel
    samples_list = [1, 2, 4, 8]

    with Pool() as pool:
        pool.map(render_scene, samples_list)
